...
def Fxuv(t, F0, t0 = 1e6, t_sat = 5e8, beta = -1.23, step_fn = False, F_final = 0, t_pms = 0, pms_factor = 1e2):
    ...
    Args:
        - t (float): time/age [s]
        - F0 (float): initial main sequence incident XUV flux [W/m2]
        - t0 (float): start time [yr]
        - t_sat (float): saturation time [yr]; change this for different stellar types (M1:500Myr, G:50Myr)
        - beta (int): exponential term [ndim]
        - step_fn (bool): True for step function from F0 to F_final [Bool]
        - F_final (float): if step_fn == True, set the final XUV flux [W/m2]
        - t_pms (float): pre-main sequence phase duration (power law decay) [yr]
        - pms_factor (float): Fxuv_pms_0/Fxuv_sat; ~1e2 for mid-to-late M stars (Ramirez & Kaltenegger 2014) [ndim]

    Returns: incident XUV flux [W/m2]
    '''
    time = t*s2yr
    if 0 < time < t_pms:
...
def Fxuv_a(t, F0, t_sat = 5e8, beta = -1.23):
    ...
    Args:
        - t (array): time/age [s; array]
        - F0 (float): initial incident XUV flux [W/m2]
        - t_sat (float): saturation time [yr]; change this for different stellar types (M1:500Myr, G:50Myr)
        - beta (int): exponential term [ndim]
    Returns: incident XUV flux [W/m2]
    '''
...
def Fxuv_Johnstone(t, d, stellar_type):
    ...
    if stellar_type == 'M1':
        path = DATADIR+'0p5Msun_50percentile_basic.dat'
    elif stellar_type == 'K5':
        path = DATADIR+'0p7Msun_50percentile_basic.dat'
    elif stellar_type == 'G5':
        path = DATADIR+'1p0Msun_50percentile_basic.dat'
...
def Fxuv_hazmat(t, d, activity):
    ...
    Args: t (time, s); d (orbital distance, m); activity ('high', 'medium', 'low')
...
def Phi_1(phi, b, H1, H2, m1, m2, x1, x2, mu, output = 0):
    ...
    Returns: number flux of light species [particles/m2/s]
    '''
...
def Phi_2(phi, b, H1, H2, m1, m2, x1, x2, mu):
    ...
    Args:
        ...
        - x1/x2: molar concentration of light/heavy species (x1=mol_1/mol_tot) [ndim]
        - mu: average atmospheric atomic mass [kg/particle]
    Returns: number flux of heavy species [particles/m2/s]
    '''
...
def R_core(Mp):
    ...
    Returns: planetary core radius [m]
    '''
...
def R_grid(Mp, f_atm, Teq, mu, k, n_tot = int(1e4)):
    ...
    Args:
        ...
        - n_tot: grid size
    Returns: radius grid of length n_tot [m]
    '''
...
def R_rcb(Mp, fatm, mu, Tsurf, Rgas = kb/mu_H2, cp = 14514, Prcb = 1e4):
    ...
    Returns: planetary radius at Prcb [m]
    '''
...
def WMF(Mp, wmf):
    ...
    Returns: number of terrestrial oceans [float]
    '''
...
def phi_kill(M_atm, Rp, age):
    ...
    Returns: mass escape flux [kg/m2/s]
    '''
    A = 4*np.pi*Rp**2
...
def b_H2_HD(T):
    ...
    Returns: Binary diffusion coefficient for H2 in HD from Genda 2008 [molecules/m/s]
    '''
...
def b_H_He(T):
    '''
    Input: T, temperature [K]
    Returns: Binary diffusion coefficient for H in He [molecules/m/s]
...
def V_reduction(Mp, Ms, a, Rp):
    ...
    Args:
     ...
     - a: orbital distance [m]
     - Rp: planet radius [m]
    Returns: Grav potential reduction factor
    '''
...
def Johnson_reduction(eps, F_xuv, Rp, Vpot):
    ...
    Args:
     ...
     - Vpot: planetary gravitational potential [J/kg]
    Returns: escape flux reduction factor, f_r
    '''
...
def f_atm_pred(Mc):
    ...
    Returns: atmopsheric mass fraction [ndim]
    '''
...
def MeltFraction(Mp, T):
    ...
    Input: 
        ...
        - Mp: planet mass [kg]
    Returns: melt fraction of the silicate layer [ndim]
    """
